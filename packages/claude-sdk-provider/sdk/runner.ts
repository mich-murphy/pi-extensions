...
export function createClaudeAgentSdkRunner(
  ...
  return async function* runClaudeAgentSdk(
    ...
    try {
      ...
      const diagnosticTurn = cacheDiagnostics?.request(
      ...
      try {
        sdkQuery = runSdkQuery({
          ...
          options: {
            ...
            hooks: {
              PreToolUse: [
                { hooks: [createPreToolUseHook(availableTools, onToolCall, onInvalidCall)] },
              ],
            },
          },
        });
      ...
      if (diagnosticTurn !== undefined && latestUsage)
        cacheDiagnostics?.usage(diagnosticTurn, latestUsage);
