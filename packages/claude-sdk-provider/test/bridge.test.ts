...
describe("serializeConversation", () => {
  test("starts each Pi turn in a stateless SDK session with the complete transcript", async () => {
    ...
    const textsOf = (message: SDKUserMessage) =>
      sdkContentRecords(message).flatMap((block) =>
...
describe("SDK query cancellation", () => {
  test("does not start an SDK query for an already-aborted signal", async () => {
    ...
    const events = await drain(
      createClaudeAgentSdkRunner(runSdkQuery)(request, model, { signal: controller.signal }),
    );
...
describe("deferred tool isolation", () => {
  test("keeps each turn's allowed-tool set independent even though the MCP schema and handler are shared module singletons", async () => {
    ...
    expect(firstTurnEvents).toEqual([
      { type: "tool_call", id: "toolu_a", name: "read", arguments: {} },
    ]);
    ...
    expect(secondTurnEvents).toEqual([
      { type: "tool_call", id: "toolu_b", name: "write", arguments: {} },
    ]);
...
describe("deferred tool retries", () => {
  test("lets the model retry after an invalid pi_call within the same query instead of ending the turn", async () => {
    ...
    expect(events).toEqual([
      { type: "tool_call", id: "toolu_good", name: "read", arguments: { path: "package.json" } },
    ]);
  });
...
describe("SDK event translation", () => {
  ...
  test("surfaces the SDK's own error result instead of silently reporting an empty stop", () => {
    ...
    if (executionError._tag === "failure")
      expect(executionError.error.message).toBe("context deadline exceeded");
    ...
    if (refusal._tag === "failure")
      expect(refusal.error.message).toBe("The model refused to respond.");
  });
...
describe("stable transcript caching", () => {
  ...
  test("forwards a toolResult message's image bytes (e.g. a screenshot tool) the same way as user images", () => {
    ...
    const request = buildAgentRequest(context);
    const entryBlock = request.promptBlocks.find((block) =>
      block.text.includes('"role":"toolResult"'),
    );
...
describe("image transcript caching", () => {
  test("keeps the stable transcript prefix (text and images) byte-identical across turns when images are present", () => {
    ...
    expect(afterPrefix.map((block) => block.images)).toEqual(
      stablePrefix.map((block) => block.images),
    );
