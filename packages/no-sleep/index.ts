...
function spawnCaffeinate(args: ReadonlyArray<string>): CaffeinateProcessResult<CaffeinateProcess> {
  try {
    ...
    return {
      ...
      value: {
        ...
        onError: (listener) =>
          child.once("error", (cause) => listener(new CaffeinateProcessError("runtime", cause))),
        onExit: (listener) => {
